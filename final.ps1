...
foreach ($job in $json.jobs) {
    ...
    $status = "UNKNOWN"
    $statusEmoji = "�s���?"
    if ($lastRunStatus -eq "ok") { $status = "OK"; $statusEmoji = "✅"; $countOK++ }
    elseif ($lastRunStatus -eq "error") { $status = "FAIL"; $statusEmoji = "❌"; $countFAIL++ }
    # Check if missed
    if ($nextRunAtMs -and $nextRunAtMs -lt $nowMs -and $lastRunAtMs -lt $nextRunAtMs) {
        $status = "MISSED"; $statusEmoji = "⏰"; $countMISSED++
    }
...
$report += "**Summary:** Total: $countTotal | Enabled: $countEnabled | ✅ OK: $countOK | ❌ FAIL: $countFAIL | ⏰ MISSED: $countMISSED`n`n"
if ($hasFailOrMissed) {
