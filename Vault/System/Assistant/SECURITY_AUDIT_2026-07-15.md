# Security Audit Report — 2026-07-15

**Audit ID:** SECURITY_AUDIT_2026-07-15
**Timestamp:** 2026-07-15 (scheduled cron execution)
...
| Category | Status | Severity | Trend vs 2026-07-14 |
|----------|--------|----------|---------------------|
| Credential Exposure | **FAIL** | CRITICAL | **Worsened** (57 .env copies, up from 49) |
| Channel Integrity | **FAIL** | CRITICAL | No Change (Telegram token invalid, gateway crash loop persists) |
| Security Events | **WARN** | HIGH | No Change (multi-provider DNS/host failure persists) |

**Overall:** **CRITICAL FAIL** — 3 CRITICAL findings, 2 persistent from prior audits, 1 **WORSENING** regression (backup `.env` proliferation increased).

---

## 1. CREDENTIAL EXPOSURE — FAIL (CRITICAL)

### 1.1 Backup `.env` Proliferation (57 copies) — WORSENING
**Locations scanned (bounded per prior audit scope):**

| Location | Count | Status |
|----------|-------|--------|
| `~/.hermes/backups/` | 35 | FAIL |
| `~/.hermes/state-snapshots/` | 4 | FAIL |
| `~/hermes-backup/` | 17 | FAIL |
| `~/.openclaw/` | 1 | FAIL |
| **Total** | **57** | **CRITICAL** |

Each `.env` contains raw API keys for: OpenRouter, Telegram, Google/Gemini, Firecrawl, Brave, xAI, BWS, Groq, Whisper, SAG, GOG. **Increased by 8 since 2026-07-14 (was 49). Persistent security debt: 10+ audit cycles.**

### 1.2 Workspace Scripts Reading `.env` Directly — RESOLVED
| File | Previous Issue | Current |
|------|----------------|---------|
| `~/.hermes/workspace/send_health_report.py` | Opened `.env`, parsed `TELEGRAM_BOT_TOKEN` | **Deleted** |
| `~/.hermes/workspace/tmp_send_telegram_test.py` | Opened `.env`, parsed `TELEGRAM_BOT_TOKEN` | **Deleted** |

Both scripts removed after prior audit. No new workspace `.env` readers detected.

### 1.3 AGENTS.md UTF-8 BOM — FAIL (Prompt Injection Risk)
- `~/.hermes/workspace/AGENTS.md`: **UTF-8 (with BOM)** — confirmed via `file` command
- BOM (`U+FEFF`) triggers prompt-injection blocks in cron execution
- **Persistent since 2026-07-02. Not remediated.**

### 1.4 Telegram Bot Token — INVALID (CRITICAL)
```
GET https://api.telegram.org/bot8277244378:***/getMe
→ HTTP 401 {"ok":false,"error_code":401,"description":"Unauthorized"}
```
...
### 1.5 Credential Caches — Present, Not Scanned This Cycle
...
| Bot token validity | **INVALID** | `getMe` → HTTP 401 |
| Channel directory topic 20 | **EXISTS** | `{"id":"-1003784520976:20","name":"Agent Hermes / topic 20","type":"group","thread_id":"20"}` |

**Critical disconnect:** Gateway holds live TCP connections but token is rejected by Telegram. Adapter believes connected; delivery fails silently.
...
- Gateway restarts under new PID but crash loop persists — **21+ days unstable**
...
### 2.4 Cron Delivery Targets — SILENT FAILURES (27/35 jobs)
...
**27 jobs (77%)** use `origin`/`local` — never reach user. Remaining 8 target Telegram topics but token is dead.
...
Prior audits (2026-07-11, 2026-07-13) found `InvalidToken: rejected by the server` in `agent.log.1` / `errors.log.1`. Current logs clean — token likely revoked **after** those entries. Confirmed by live `getMe` 401.
...
## 4. TREND COMPARISON (vs 2026-07-14 Audit)

| Finding | 2026-07-14 | 2026-07-15 | Trend | Remediation Status |
|---------|------------|------------|-------|-------------------|
| Backup `.env` copies | 49 | **57** | **↑ Worsening** | **Not Remediated** (10+ cycles) |
| Workspace `.env` readers | 2 | 0 | ↓ Improved | **Resolved** (scripts deleted) |
| AGENTS.md BOM | FAIL | FAIL | ↔ No Change | **Not Remediated** |
| Telegram token | Invalid (404) | Invalid (401) | ↔ No Change | **Critical — Rotate Now** |
| Gateway crash loop | 5 entries | 5 entries | ↔ No Change | **Not Fixed** (missing dep) |
| WhatsApp unpaired | 65+ days | 65+ days | ↔ No Change | **Not Remediated** |
| Cron silent delivery | 27/40 jobs | 27/35 jobs | ↔ No Change | **Not Remediated** |

**Persistent Security Debt (≥3 cycles unremediated):**
1. Backup `.env` proliferation (57 copies) — **ESCALATED to CRITICAL (worsening)**
...
| **P0** | Delete 57 backup `.env` copies outside `~/.hermes/.env` | Agent (next cron) | 2 min |
...
- Token status: ❌ Invalid (HTTP 401)
- Delivery method: Direct Telegram Bot API (Pattern A: write_file to Windows temp + python3.11 exec)

**Note:** This summary is emitted in the cron job response for automated delivery. If gateway delivery fails (token invalid), the report artifact remains at `C:\Users\User\.hermes\workspace\Vault\System\Assistant\SECURITY_AUDIT_2026-07-15.md` for manual retrieval.

---

## 7. ARTIFACT RETENTION

- **Keep:** This report (`SECURITY_AUDIT_2026-07-15.md`)
- **Delete:** Prior same-day variants (none this cycle)
- **Retention window:** 7 days rolling (delete `SECURITY_AUDIT_2026-07-08.md` and older)
