...
The Kanban dispatcher runs **inside the Hermes gateway process** (not as a separate server). If your task is in `ready` state but no worker is spawned:

1. Check the gateway is running: `ps aux | grep hermes` or `tasklist | grep hermes`
2. Check gateway logs for dispatcher status: `grep -i kanban ~/.hermes/logs/gateway.log | tail -5`
3. Look for `crashed > 0` — if workers keep crashing, the dispatcher may auto-block further spawns
4. `hermes gateway restart` to reset dispatcher state

If the dispatcher log shows `spawned=0` and `auto_blocked > 0`, the dispatcher has stopped spawning due to repeated failures. A gateway restart clears this.
