...
## First-Time Setup (Before You Can Route Anything)

When Kanban isn't running yet, do this before any decomposition:

### 1. Initialize the database
```bash
hermes kanban init
```
This creates `~/.hermes/kanban.db` and discovers available profiles. Idempotent — safe to run if already initialized.

### 2. Verify the gateway is running
```bash
hermes gateway status
```
The embedded dispatcher lives in the gateway. Without it, `ready` tasks sit forever. If not running: `hermes gateway start`.

### 3. Check available profiles
```bash
hermes profile list
```
These are your valid `--assignee` values. The dispatcher silently drops unknown names.

### 4. Check current board state
```bash
hermes kanban boards list
hermes kanban stats
```

### 5. Import tasks from a file (if migrating from markdown)

If the user has a `tasks-queue.md`, `TASKS.md`, or similar file with `[ ]` items:

```bash
# One-liner per task:
hermes kanban create "Task title" --assignee <profile> --body "Context, project category, notes"
```

Batch-create in groups of 5–6 to avoid timeout. Capture the returned task IDs. For dependency chains, create parents first, then children with `--parent <id>`.

### 6. Set up sync cron jobs (if file is a living source)

If the user wants a markdown file to stay in sync with the board:

- **File → Kanban sync**: Create a cron that reads the file, compares with `hermes kanban list`, creates new cards for `[ ]` items, completes cards for `[x]` items.
- **Brain-dump → File parser**: Create a cron that scans a Telegram topic for raw dumps, extracts structured tasks, appends to the file.

See `references/brain-dump-pipeline.md` for the full pipeline pattern.

### 7. Verify the pipeline
```bash
hermes kanban list
```
Confirm tasks appear in `ready` or `running`. The dispatcher picks up `ready` tasks every ~60 seconds.
...
When the user reports "Failed to load Kanban board" or a JSON parse error like `Unexpected token '<', "<!doctype "...`, the dashboard is receiving HTML instead of JSON from the API. This is almost always a gateway serving issue, not a Kanban data issue.

**Key fact:** With `dispatch_in_gateway: true`, Kanban runs *inside* the gateway process. There is no standalone Kanban HTTP server. The dashboard UI and API are served by the gateway itself.

Diagnostic steps and common causes: see `references/dashboard-api-debugging.md`.

Quick checklist:
1. Is the gateway process running? (`ps aux | grep hermes` or `tasklist | grep hermes`)
2. Is the kanban dispatcher initializing? (`grep -i kanban ~/.hermes/logs/gateway.log | tail -5` — look for `spawned > 0`)
3. Has it been ~60s since gateway restart? (dispatcher needs time to init)
4. Try `hermes gateway restart` and wait 60s

## Pitfalls

**Dashboard returns HTML instead of JSON.** When `dispatch_in_gateway: true`, the dashboard API is served by the gateway process. If you see `<!doctype` in JSON parse errors, the gateway either isn't running, hasn't finished initializing the dispatcher, or the HTTP endpoint isn't being served. Check gateway logs and restart if needed. See `references/dashboard-api-debugging.md`.

**User wants operational wiring, not plans.** When the user says "I want to start using this" or "wire this up", they want the full pipeline live — not a design doc. Set up the DB, create the tasks, configure the crons, verify it works. Then report what's running.

**No first-time setup guidance.** The orchestrator skill assumes Kanban is already running. If `hermes kanban init` hasn't been run, the gateway isn't started, or no profiles exist, the decomposition will silently fail. Always run the setup checklist (init → gateway → profiles → boards → import → sync crons) before fanning out work.

**Importing from markdown files.** When migrating tasks from a file (`tasks-queue.md`, `TASKS.md`, etc.): batch-create in groups of 5–6 via `hermes kanban create` to avoid terminal timeouts. Always pass `--body` with project context. Always `hermes kanban list` first to avoid duplicates.
