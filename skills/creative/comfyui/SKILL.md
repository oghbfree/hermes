...
- User asks to generate images with Stable Diffusion, SDXL, Flux, SD3, etc. **AND** has a local NVIDIA GPU or Comfy Cloud subscription
- User wants to run a specific ComfyUI workflow file
- User needs ControlNet, inpainting, img2img, or other advanced pipelines not available via simpler tools
- User asks to manage ComfyUI queue, check models, or install custom nodes
- User wants video/audio/3D generation via AnimateDiff, Hunyuan, Wan, AudioCraft, etc.

> **⚡ Preferred alternative:** For most image generation tasks, use the built-in `image_gen` tool (FAL.ai backend) instead. It's faster, requires no local GPU, and has a free tier. Use ComfyUI only when you need advanced workflow features (ControlNet, multi-step pipelines, custom nodes) that FAL.ai doesn't support.
...
12. **Workflow file names are specific** — the skill's `workflows/` directory
    contains specific files. The Flux Dev workflow is `flux_dev_txt2img.json`
    (NOT `flux_dev_fp8_cloud.json` — that file does not exist in this skill).
    Always check `workflows/` for available files before referencing a workflow
    name in a prompt or script. Run `ls workflows/` to list them.

13. **Comfy Cloud free tier blocks all execution** — the free tier returns
    HTTP 429 PAYMENT_REQUIRED for `/api/prompt`, `/api/upload/*`, and
    `/api/view`. A paid subscription (Standard ~$10/mo) is required for ANY
    image generation via the Cloud API. The skill will surface this clearly
    when it happens, but check proactively before building pipelines that
    depend on Cloud generation.

14. **Character consistency for AI influencers** — When generating images of
    a recurring AI character, always include the FULL character description
    in every prompt. Key elements that drift: earring color (specify exact
    metal color), hair accessories (mention headbands/bands explicitly),
    skin texture (specify "visible pores, natural vellus hair, NO beauty
    filters"), and body proportions. Include a comprehensive negative prompt
    that explicitly bans common drift patterns (e.g., "no silver earrings"
    if the character wears gold).

15. **CPU-only machines cannot run modern models** — ComfyUI on CPU (no NVIDIA GPU) is technically possible but impractically slow for Flux/SDXL (5-15 min/image). On CPU-only Windows machines, use the built-in `image_gen` tool (FAL.ai) instead. The `hardware_check.py` script will return verdict `cloud` for these machines — respect it.
