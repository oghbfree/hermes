...
Run `scripts/setup.sh` to verify all dependencies. Requires: Python 3.10+, Manim Community Edition v0.20+ (`pip install manim`), LaTeX (`texlive-full` on Linux, `mactex` on macOS, `miktex` on Windows), and ffmpeg.

### Windows-Specific: MSVC Build Tools Required

On Windows, Manim CE requires `moderngl` which needs a C compiler. If `pip install manim` fails with `error: Microsoft Visual C++ 14.0 or greater is required`:

1. Download Build Tools from https://aka.ms/vs/17/release/vs_buildtools.exe
2. Run: `vs_buildtools.exe --quiet --wait --add Microsoft.VisualStudio.Workload.VCTools --add Microsoft.VisualStudio.Component.Windows10SDK.19041 --includeRecommended`
3. Wait 15-30 minutes for download/install (~8GB)
4. Verify: `cl.exe` should exist at `C:\Program Files (x86)\Microsoft Visual Studio\2022\BuildTools\VC\Tools\MSVC\14.44.35207\bin\Hostx64\x64\`
5. Then: `pip install moderngl glcontext` (set INCLUDE/LIB env vars if needed)
6. Then: `pip install manim`

**Note:** Python 3.14 has no pre-built moderngl wheels — MSVC Build Tools are mandatory. The install took ~30 min on a standard connection.

### ⚠️ Windows + Python 3.14: moderngl Compile Block

**Problem:** Manim depends on `moderngl` which requires a C compiler and OpenGL headers. On Python 3.14, there are NO pre-built wheels for `moderngl` or `glcontext`. `pip install manim` will fail with "Microsoft Visual C++ 14.0 or greater is required."

**Fix path (in order of preference):**

1. **Use Python 3.11-3.13** (recommended): Create a venv with an older Python version that has pre-built moderngl wheels:
   ```bash
   py -3.11 -m venv .venv
...
2. **Install MSVC Build Tools** (if Python 3.14 is required):
   - Download from https://visualstudio.microsoft.com/visual-cpp-build-tools/
   - Install "Desktop development with C++" workload
   - Also install Windows 10 SDK (10.0.19041.0 or later)
   - After install, set env vars:
     ```bash
     export PATH="/c/Program Files (x86)/Microsoft Visual Studio/2022/BuildTools/VC/Tools/MSVC/<version>/bin/Hostx64/x64:$PATH"
     export INCLUDE="<MSVC include>;<Windows Kits include>"
     export LIB="<MSVC lib>;<Windows Kits lib>"
     ```
   - Then: `pip install moderngl glcontext` followed by `pip install manim`

3. **Use WSL2** (alternative): Run Manim in Ubuntu under WSL2 where `pip install manim` works without MSVC.

**Status:** As of May 2026, Python 3.14 + Windows + Manim is blocked without MSVC Build Tools. The `ai-influencer-content-pipeline` skill notes this as pitfall #14 and falls back to Hyperframes for video content until Manim is available.
