...
HyperFrames has no native still-export command, but extracting frames is trivial:

```bash
# Grab a single frame from a rendered video:
ffmpeg -i output.mp4 -ss 00:00:03 -vframes 1 still.png
```

For **high-quality stills without rendering video first**, screenshot the composition HTML directly via Puppeteer (which HyperFrames uses internally). This is faster and avoids video encoding overhead. Use this approach for:
- Product card images (Jiji, FB Marketplace, WhatsApp Status)
- Social media post graphics
- Thumbnail generation

The `validate` and `inspect` commands already screenshot compositions internally — the same mechanism can be used for intentional still capture.
...
- **GSAP/CSS transform conflict** — Never set `transform` in CSS on elements you animate with GSAP. GSAP overwrites the full CSS transform on every frame, so a CSS `transform: translateY(200px)` combined with a GSAP `y` tween will discard the CSS value. Instead, put the start state in GSAP: use `tl.fromTo(el, { y: 200 }, { y: 0, ... })` rather than CSS `transform: translateY(200px)` + `tl.to(el, { y: 0 })`. This applies to `scale`, `rotate`, `x`, `y` — all transform sub-properties.
- **Scene layers need visibility kills** — When exiting a scene via opacity tween, always add a hard `tl.set("#scene-id", { visibility: "hidden" }, <exit-end-time>)` after the opacity tween completes. Without this, the invisible scene can overlap and occlude later scenes during scrubbing or tween conflicts. This applies to every scene layer except the final one.
...
- [windows-gotchas.md](references/windows-gotchas.md) — Windows/MSYS path handling, render notes, lint warning guide
