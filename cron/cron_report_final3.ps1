...
function Convert-CronToHuman($expr) {
    ...
    function Format-Time { param($h, $m) return "{0:D2}:{1:D2}" -f [int]$h, [int]$m }
    
    # Daily at time
    if ($min -match '^\d+$' -and $hour -match '^\d+$' -and $dom -eq '*' -and $mon -eq '*' -and $dow -eq '*') {
        return "Daily at $(Format-Time $hour $min)"
    }
    # Weekly on specific day(s)
    if ($min -match '^\d+$' -and $hour -match '^\d+$' -and $dom -eq '*' -and $mon -eq '*') {
        $dayNames = @('Sun','Mon','Tue','Wed','Thu','Fri','Sat')
        if ($dow -match '^\d+$') {
            return "Weekly on $($dayNames[[int]$dow]) at $(Format-Time $hour $min)"
        } elseif ($dow -match '^\d+-\d+$') {
            $start, $end = $dow -split '-'
            $startName = $dayNames[[int]$start]
            $endName = $dayNames[[int]$end]
            # Assume consecutive days
            if ($end - $start -eq 1) {
                return "$startName-$endName at $(Format-Time $hour $min)"
            } else {
                return "Days $dow at $(Format-Time $hour $min)"
            }
        } elseif ($dow -match ',') {
            $days = $dow -split ',' | ForEach-Object { $dayNames[[int]$_] }
            return "$($days -join ',') at $(Format-Time $hour $min)"
        }
    }
    # Monthly on day
    if ($min -match '^\d+$' -and $hour -match '^\d+$' -and $dom -match '^\d+$' -and $mon -eq '*' -and $dow -eq '*') {
        return "Monthly on day $dom at $(Format-Time $hour $min)"
    }
    # Hourly
